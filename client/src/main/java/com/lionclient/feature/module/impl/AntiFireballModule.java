...
public final class AntiFireballModule extends Module {
    private static AntiFireballModule instance;
    public static AntiFireballModule getInstance() { return instance; }
    ...
    private void seedTrackedFireballs() {
        ...
        int size = minecraft.theWorld.loadedEntityList.size();
        for (int i = 0; i < size; i++) {
            Object object = minecraft.theWorld.loadedEntityList.get(i);
    ...
    private float[] computeAimRotations(Minecraft minecraft, float baseYaw, float basePitch) {
        ...
        int maxPlayers = 10;
        EntityPlayer[] topPlayers = new EntityPlayer[maxPlayers];
        double[] topDistances = new double[maxPlayers];
        int count = 0;

        int size = minecraft.theWorld.playerEntities.size();
        for (int i = 0; i < size; i++) {
            EntityPlayer player = minecraft.theWorld.playerEntities.get(i);
            ...
            for (int j = 0; j < count; j++) {
                if (distSq < topDistances[j]) {
                    insertPos = j;
                    break;
                }
            }
            
            if (insertPos < maxPlayers) {
                int shiftEnd = Math.min(count, maxPlayers - 1);
                for (int j = shiftEnd; j > insertPos; j--) {
                    topPlayers[j] = topPlayers[j - 1];
                    topDistances[j] = topDistances[j - 1];
                }
                topPlayers[insertPos] = player;
                topDistances[insertPos] = distSq;
                if (count < maxPlayers) count++;
            }
        }

        for (int i = 0; i < count; i++) {
            EntityPlayer player = topPlayers[i];
