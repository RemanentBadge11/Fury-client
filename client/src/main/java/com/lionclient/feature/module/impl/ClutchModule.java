...
public final class ClutchModule extends Module {
    ...
    public enum BlockSelectMode {
        ...
        private final String label;
        BlockSelectMode(String label) { this.label = label; }
        @Override public String toString() { return label; }
    }
    ...
    private final BooleanSetting midAirOnly = new BooleanSetting("Mid-Air Only", true);
    private final BooleanSetting recentDamageOnly = new BooleanSetting("Recent Damage Only", false);
    private final BooleanSetting backwardsMotionOnly = new BooleanSetting("Backwards Motion Only", false);

    // Block Selection & Filter Settings
    private final EnumSetting<BlockSelectMode> selectBlocks = new EnumSetting<BlockSelectMode>("Select Blocks", BlockSelectMode.values(), BlockSelectMode.ALWAYS);
    private final EnumSetting<FilterMode> filterMode = new EnumSetting<FilterMode>("Filter Mode", FilterMode.values(), FilterMode.NONE);

    // Placement Rules Settings
    private final BooleanSetting onlyPlaceSideways = new BooleanSetting("Only Place Sideways", false);
    private final NumberSetting minHeight = new NumberSetting("Min Height", 1, 10, 1, 2);
    private final NumberSetting maxBlocks = new NumberSetting("Max Blocks", 1, 64, 1, 10);
    private final DecimalSetting range = new DecimalSetting("Range", 3.0D, 6.0D, 0.1D, 4.5D);
    private final NumberSetting fov = new NumberSetting("FOV", 30, 360, 5, 360);
    private final NumberSetting clickSpeed = new NumberSetting("Click Speed", 1, 20, 1, 15);
    private final NumberSetting randomization = new NumberSetting("Randomization", 0, 100, 5, 10);

    // Aim & Rotation Settings (Vape / Slinky Style)
    private final DecimalSetting baseSpeed = new DecimalSetting("Base Speed", 10.0D, 180.0D, 1.0D, 60.0D);
    private final NumberSetting acceleration = new NumberSetting("Acceleration", 0, 100, 5, 30);
    private final NumberSetting accelerationStrength = new NumberSetting("Acceleration Strength", 0, 100, 5, 50);
    ...
    // Snap-Back & Post-Clutch Settings
    private final BooleanSetting rotateBack = new BooleanSetting("Snap-back", true);
    private final NumberSetting snapBackDelay = new NumberSetting("Snap-back Delay", 0, 20, 1, 1);
    private final NumberSetting snapBackDuration = new NumberSetting("Snap-back Duration", 1, 20, 1, 3);
    private final BooleanSetting keepJumpDirection = new BooleanSetting("Keep Jump Direction", true);
    ...
    private final BooleanSetting allowStaircaseUp = new BooleanSetting("Allow Staircase Up", true);
    ...
    private int snapBackProgressTicks;
    ...
    private ClutchModule() {
        super("Clutch", "Automatically places blocks underneath to prevent fatal falls.", Category.PLAYER, Keyboard.KEY_NONE);
        ...
        snapBackDelay.setVisibility(new java.util.function.BooleanSupplier() {
            @Override
            public boolean getAsBoolean() {
                return rotateBack.isEnabled() && !silentAim.isEnabled();
            }
        });
        snapBackDuration.setVisibility(new java.util.function.BooleanSupplier() {
            @Override
            public boolean getAsBoolean() {
                return rotateBack.isEnabled() && !silentAim.isEnabled();
            }
        ...
        addSetting(midAirOnly);
        addSetting(recentDamageOnly);
        addSetting(backwardsMotionOnly);
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (player.isInWater() || player.isInLava()) {
            if (clutching || rotationActive) {
                finishClutch(player);
            }
            return;
        }

        // Check keepJumpDirection (Telly scaffold style snap-back on jump)
        if (keepJumpDirection.isEnabled() && player.movementInput != null && player.movementInput.jump && clutching) {
            finishClutch(player);
            return;
        }
        ...
        if (midAirOnly.isEnabled() && player.onGround) {
            return;
        }

        if (player.motionY >= 0.0D && !allowStaircaseUp.isEnabled()) {
            return;
        }

        if (recentDamageOnly.isEnabled() && player.hurtTime <= 0) {
            return;
        }

        if (backwardsMotionOnly.isEnabled() && !isMovingBackwards(player)) {
            return;
        }

        if (getAirHeightBelow(player) < minHeight.getValue()) {
            return;
        }
        ...
        if (!clutching) {
            ...
            initialStackSize = (held != null && held.getItem() instanceof ItemBlock) ? held.stackSize : 0;
        }
        ...
        if (!isAngleInFOV(player, aim[0], aim[1])) {
            return;
        }

        setRotationTarget(aim[0], aim[1]);
        stepRotation(computeEffectiveRotationSpeed());
        applyVisibleRotation();

        if (silentAim.isEnabled()) {
            ClientRotationHelper.get().updateSilentYaw(currentYaw, currentPitch);
        }

        if (!hasReachedTarget(3.0F)) {
            rotationHeldTicks = 0;
            return;
        }
        ...
        // Check click speed limit
        long now = System.currentTimeMillis();
        long minClickInterval = 1000L / Math.max(1, clickSpeed.getValue());
        if (now - lastClickTime < minClickInterval) {
            return;
        }

        MovingObjectPosition hit = rayTraceAtRotation(player, range.getValue(), currentYaw, currentPitch);
        if (hit == null
        ...
        if (onlyPlaceSideways.isEnabled() && (hit.sideHit == EnumFacing.UP || hit.sideHit == EnumFacing.DOWN)) {
            return;
        }
    ...
    private void handleGroundState(EntityPlayerSP player) {
        ...
        snapBackProgressTicks++;
        int totalTicks = Math.max(1, snapBackDuration.getValue());
        float progress = Math.min(1.0F, (float) snapBackProgressTicks / totalTicks);

        stepRotation(computeEffectiveRotationSpeed() * (1.0F + progress));
        applyVisibleRotation();

        if (hasReachedTarget(2.0F) || progress >= 1.0F) {
            clearRotationState();
            returningToCamera = false;
        }
    }
    ...
    private void finishClutch(EntityPlayerSP player) {
        ...
        moveFreezeTicks = Math.max(0, clutchMoveDelay.getValue());
        clutching = false;
        blocksPlaced = 0;
    ...
    private int findBlockSlot(EntityPlayerSP player) {
        for (int slot = 0; slot <= 8; slot++) {
            ...
            if (filterMode.getValue() == FilterMode.BLACKLIST && isBlacklistedBlock(block)) {
                continue;
            }
            if (filterMode.getValue() == FilterMode.WHITELIST && !isWhitelistedBlock(block)) {
                continue;
            }
    ...
    private boolean isBlacklistedBlock(Block block) {
        ...
        return name.contains("tnt") || name.contains("anvil") || name.contains("sand") || name.contains("gravel") || name.contains("glass");
    }

    private boolean isWhitelistedBlock(Block block) {
        String name = block.getUnlocalizedName().toLowerCase();
        return name.contains("cloth") || name.contains("wood") || name.contains("stone") || name.contains("brick");
    }

    private boolean ensureHoldingBlock(EntityPlayerSP player) {
        ItemStack heldItem = player.getHeldItem();
        boolean holdingValidBlock = heldItem != null && heldItem.getItem() instanceof ItemBlock;

        if (selectBlocks.getValue() == BlockSelectMode.NO) {
            return holdingValidBlock;
        }

        if (selectBlocks.getValue() == BlockSelectMode.ON_DEPLETION) {
            if (holdingValidBlock) {
                return true;
            }
    ...
    private PlacementCandidate findBridgePlacement(EntityPlayerSP player) {
        ...
        while (bridgeIndex < bridgePath.size()) {
            BlockPos targetPos = bridgePath.get(bridgeIndex);
            ...
            if (bridgeIndex == 0 && bridgeStartPlacement != null) {
                ...
                placement = null;
                if (bridgeIndex > 0) {
                    placement = makePlacementAgainst(targetPos, bridgePath.get(bridgeIndex - 1));
                }
                if (placement == null) {
                    placement = findPlacementInfo(targetPos);
                }
            }
            ...
            if (!isPlacementReachable(player, placement.neighbor, placement.face, range.getValue())) {
                return null;
            }
    ...
    private float[] faceAim(EntityPlayerSP player, BlockPos neighbor, EnumFacing face) {
        ...
        if (multipoint.isEnabled()) {
            ...
            hitX = MathHelper.clamp_double(playerX, neighbor.getX() + 0.05D, neighbor.getX() + 0.95D);
            hitZ = MathHelper.clamp_double(playerZ, neighbor.getZ() + 0.05D, neighbor.getZ() + 0.95D);
        }
        ...
        // Add randomization jitter if enabled
        if (randomization.getValue() > 0) {
            float jitter = (float) ((Math.random() - 0.5D) * (randomization.getValue() / 50.0D));
            yaw += jitter;
            pitch += jitter * 0.5F;
        }
    ...
    private float computeEffectiveRotationSpeed() {
        float speed = (float) baseSpeed.getValue();
        if (!rotationActive) {
            return speed;
        }
        ...
        float remainingAngle = MathHelper.sqrt_float(yawDiff * yawDiff + pitchDiff * pitchDiff);

        float accelFactor = (acceleration.getValue() / 100.0F) * (remainingAngle / 180.0F) * 120.0F;
        float weight = accelerationStrength.getValue() / 100.0F;

        return speed * (1.0F - weight) + accelFactor * weight;
    }

    private boolean isAngleInFOV(EntityPlayerSP player, float yaw, float pitch) {
        float fovVal = fov.getValue();
        if (fovVal >= 360) return true;
        ...
        return yawDiff <= fovVal / 2.0F && pitchDiff <= fovVal / 2.0F;
    }
    ...
    private boolean isAttachableBlock(BlockPos pos) {
        ...
        if (material == Material.air || material.isLiquid() || block instanceof BlockLiquid) {
            return false;
        }
        for (EnumFacing face : EnumFacing.values()) {
            Block adj = mc.theWorld.getBlockState(pos.offset(face)).getBlock();
            if (adj instanceof BlockLiquid || adj.getMaterial().isLiquid()) {
                return false;
            }
        }
        return true;
            ...
            for (int dx = -searchRange; dx <= searchRange; dx++) {
                for (int dz = -searchRange; dz <= searchRange; dz++) {
                    ...
                    for (EnumFacing face : EnumFacing.values()) {
                        ...
                        if (!isPlacementReachable(player, solidPos, face, r)) {
                            continue;
                        }
    ...
    private int getAirHeightBelow(EntityPlayerSP player) {
        int feetY = MathHelper.floor_double(player.posY);
        int playerX = MathHelper.floor_double(player.posX);
        int playerZ = MathHelper.floor_double(player.posZ);

        int airHeight = 0;
        for (int y = feetY - 1; y >= 0; y--) {
            if (isSupportBlock(new BlockPos(playerX, y, playerZ))) {
                break;
            }
            airHeight++;
    ...
    private boolean isMovingBackwards(EntityPlayerSP player) {
        if (player == null) return false;
        double motionX = player.motionX;
        double motionZ = player.motionZ;
        if (motionX * motionX + motionZ * motionZ < 1.0E-4D) return false;

        double yawRad = Math.toRadians(player.rotationYaw);
        double forwardX = -Math.sin(yawRad);
        double forwardZ = Math.cos(yawRad);
        double proj = motionX * forwardX + motionZ * forwardZ;
