...
public final class TrajectoriesModule extends Module {
    ...
    public void onRenderWorld(RenderWorldLastEvent event) {
        ...
        if (result.entityHit == null) {
            writeTrajectoryColor(colorScratch);
        } else {
            writeAimingColor(colorScratch);
        }
        GL11.glPushMatrix();
        try {
            ...
            renderPath(minecraft, result.points, colorScratch);
        } finally {
    ...
    private static boolean isProjectile(Item item) {
        return item instanceof ItemBow || item instanceof ItemFishingRod || item instanceof ItemPotion
                || item instanceof ItemSnowball || item instanceof ItemEgg || item instanceof ItemEnderPearl;
    }
    ...
    private SimulationResult simulatePath(Minecraft minecraft, EntityPlayerSP player, ProjectileProperties properties, float partialTicks) {
        List<Vec3> points = new ArrayList<Vec3>(64);
        ...
        int maxSteps = calculateMaxSteps(properties.velocity);
    ...
    private int calculateMaxSteps(double velocity) {
        int baseSteps = 150;
        int velocitySteps = (int) (velocity * 15);
        return Math.min(baseSteps + velocitySteps, 300);
    }
    ...
    private void writeAimingColor(float[] out) {
        out[0] = aimingRed.getValue() / 255.0F;
        out[1] = aimingGreen.getValue() / 255.0F;
        out[2] = aimingBlue.getValue() / 255.0F;
    }

    private void writeTrajectoryColor(float[] out) {
        out[0] = trajectoryRed.getValue() / 255.0F;
        out[1] = trajectoryGreen.getValue() / 255.0F;
        out[2] = trajectoryBlue.getValue() / 255.0F;
    }
