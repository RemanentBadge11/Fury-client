...
public final class KillAuraRotationUtils {
    ...
    private static final Vec3[] BACKUP_POINTS_BUFFER = new Vec3[40];

    private static int buildBackupPoints(Entity entity, Vec3 eye) {
        if (entity == null || MINECRAFT.thePlayer == null) {
            return 0;
        }
        ...
        if (xPos || xNeg) {
            ...
            count = addFaceGrid(count, 0, fixedX, bb.minY + BACKUP_FACE_INSET, bb.maxY - BACKUP_FACE_INSET, bb.minZ + BACKUP_FACE_INSET, bb.maxZ - BACKUP_FACE_INSET, pointsPerFace, sizeY, sizeZ);
        }
        if (yPos || yNeg) {
            double fixedY = yPos ? bb.maxY - BACKUP_FACE_INSET : bb.minY + BACKUP_FACE_INSET;
            count = addFaceGrid(count, 1, fixedY, bb.minX + BACKUP_FACE_INSET, bb.maxX - BACKUP_FACE_INSET, bb.minZ + BACKUP_FACE_INSET, bb.maxZ - BACKUP_FACE_INSET, pointsPerFace, sizeX, sizeZ);
        }
        if (zPos || zNeg) {
            double fixedZ = zPos ? bb.maxZ - BACKUP_FACE_INSET : bb.minZ + BACKUP_FACE_INSET;
            count = addFaceGrid(count, 2, fixedZ, bb.minX + BACKUP_FACE_INSET, bb.maxX - BACKUP_FACE_INSET, bb.minY + BACKUP_FACE_INSET, bb.maxY - BACKUP_FACE_INSET, pointsPerFace, sizeX, sizeY);
        }

        return count;
    }

    private static int addFaceGrid(int startCount, int fixedAxis, double fixedValue, double uMin, double uMax, double vMin, double vMax, int targetPoints, double dimU, double dimV) {
        int count = startCount;
        if (dimU < 1.0E-4D || dimV < 1.0E-4D) {
            double uMid = (uMin + uMax) / 2.0D;
            double vMid = (vMin + vMax) / 2.0D;
            if (count < BACKUP_POINTS_BUFFER.length) {
                if (fixedAxis == 0) {
                    BACKUP_POINTS_BUFFER[count++] = new Vec3(fixedValue, uMid, vMid);
                } else if (fixedAxis == 1) {
                    BACKUP_POINTS_BUFFER[count++] = new Vec3(uMid, fixedValue, vMid);
                } else {
                    BACKUP_POINTS_BUFFER[count++] = new Vec3(uMid, vMid, fixedValue);
                }
            }
            return count;
        }
        ...
        for (int i = 0; i < gridU; i++) {
            ...
            for (int j = 0; j < gridV; j++) {
                ...
                if (count < BACKUP_POINTS_BUFFER.length) {
                    if (fixedAxis == 0) {
                        BACKUP_POINTS_BUFFER[count++] = new Vec3(fixedValue, u, v);
                    } else if (fixedAxis == 1) {
                        BACKUP_POINTS_BUFFER[count++] = new Vec3(u, fixedValue, v);
                    } else {
                        BACKUP_POINTS_BUFFER[count++] = new Vec3(u, v, fixedValue);
                    }
                }
            }
        }
    ...
    public static boolean hasValidAimPoint(Entity entity, double horizontalMultipoint, double verticalMultipoint, double range, boolean allowThroughBlocks, boolean allowThroughEntities) {
        ...
        int size = buildBackupPoints(entity, eye);
        for (int i = 0; i < size; i++) {
            int closestIndex = i;
            double minSqDist = Double.MAX_VALUE;
            for (int j = i; j < size; j++) {
                Vec3 p = BACKUP_POINTS_BUFFER[j];
            ...
            Vec3 closestPoint = BACKUP_POINTS_BUFFER[closestIndex];
            BACKUP_POINTS_BUFFER[closestIndex] = BACKUP_POINTS_BUFFER[i];
            BACKUP_POINTS_BUFFER[i] = closestPoint;

            if (canAimAtPoint(eye, closestPoint, entity, range, allowThroughBlocks, allowThroughEntities)) {
                return true;
            }
        }
        return false;
    ...
    public static float[] getRotationsWithBackup(Entity entity, double horizontalMultipoint, double verticalMultipoint, float baseYaw, float basePitch, double range, boolean allowThroughBlocks, boolean allowThroughEntities) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (canAimAtPoint(eye, closestPoint, entity, range, allowThroughBlocks, allowThroughEntities)) {
                return getRotationsToPoint(closestPoint.xCoord, closestPoint.yCoord, closestPoint.zCoord, baseYaw, basePitch);
            }
        }
        return null;
    }
